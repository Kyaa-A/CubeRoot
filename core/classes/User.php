...
class User extends Connect {
    ...
    public static function login($email, $password) {
        $stmt = self::connect()->prepare("SELECT `id`, `role` FROM `users` WHERE `email` = :email AND `password` = :password");
        ...
        if ($stmt->rowCount() > 0) {
            ...
            if ($user->role == 2) {
                header('location: ../admin_panel.php');  // Admin redirect
            } else {
                header('location: ../home.php');  // Regular user redirect
            }
        } else {
            return false; 
        }
    ...
    public static function register($email, $password, $name, $username) {
        ...
        // make user follow the owner by default and notifications also
    ...
    public static function search($search) {
        $stmt = self::connect()->prepare("SELECT `id`, `username`, `name`, `img`, `imgCover` FROM `users` WHERE `username` LIKE ? OR `name` LIKE ?");
    ...
    public static function CountNotification($user_id) {
        $stmt = self::connect()->prepare("SELECT COUNT(notify_for) as count FROM `notifications` WHERE notify_for = :user_id AND count = 0");
    ...
    public static function notification($user_id) {
        $stmt = self::connect()->prepare("SELECT * FROM `notifications` WHERE notify_for = :user_id ORDER BY time DESC");
        $stmt->bindParam(":user_id", $user_id, PDO::PARAM_STR);
        $stmt->execute();
        return $stmt->fetchAll(PDO::FETCH_OBJ);
    } 
    ...
    public static function updateNotifications($user_id) {
        $stmt = self::connect()->prepare("UPDATE `notifications` SET count = 1 WHERE notify_for = :user_id AND count = 0");
        $stmt->bindParam(":user_id", $user_id, PDO::PARAM_STR);
        $s = $stmt->execute();
        return $s ? true : false;  
    } 
